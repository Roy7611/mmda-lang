...
> 已拍定决策见 §0，未决项见 §6，**2026-09-24 的三十三项裁决见 §6.3**。
...
26. **嵌入式脚本执行引擎（DB 触发器的可移植替代）**：钩子（`beforeXxx` / `afterXxx`，住 Service，见 [`doc/runtime.md`](doc/runtime.md) §4.2）里能写什么——**A** 受限脚本 = **m 语言受限子集**、由 **Rust 内核**求值、走 **P6 嵌入通道**（Panama / P-Invoke + WASM，**助手建议**）；**B** 宿主脚本引擎（Java GraalJS／C# Roslyn·Jint）；**C** 不做脚本、只留声明式。技术事实已核入 [`doc/runtime.md`](doc/runtime.md) **§4.5**（Nashorn 随 JDK 15 移除 JEP 372、GraalJS 不保证 stock JDK、**引擎限额 ≠ 隔离边界**）。**待答两问**：取哪档？取 A 时首版脚本能力清单定到哪一档。
