...
> 已拍定决策见 §0，未决项见 §6，**2026-09-24 的三十二项裁决见 §6.3**。
...
| 32 | **事件总线与集成编排 9 条裁决（`event_bus.md` §15；仅第 3 条待定）**——**作者原话：「1A, 2B, 4A, 5A, 6A,7A,8C, 9A端点是要在API的基础上增加定义数据的转化、过滤规则的」+「3 待定」**：① **§15-1 取 A**：执行层 = **内嵌轻量执行器**（随应用部署、语义逐条对齐 Flink），`flink` 留 P10、**第三方消息框架不做**；② **§15-2 取 B**（**与助手建议的 2A 相反**）：**外部端点的定义反向导入成语言声明**——走 `api.md` §6 同一条护栏（**报告 + 骨架 + 人审后入真源**），**外部 collection 本身仍不是真源**、变化靠**再导入 + diff**（不自动跟随）；③ **§15-3 待定**：数据流图元落点（`mf-flow`/`mf-map` view 种类 / 新扩展名 `*.mx` / 只放 Profile）——**未定不许进生成器**；④ **§15-4 取 A**：端到端一致性只写**「状态 exactly-once + 汇端幂等」**（诚实档）；⑤ **§15-5 取 A**：**多租户 = 共享执行 + 租户键**（B/C 按客户；**同时收口 `operations.md` §9 同一三档**）；⑥ **§15-6 取 A**：迟到数据进**侧输出 / 迟到队列**（可配丢弃或补算）；⑦ **§15-7 取 A**：数据映射**可含**外部报文形态（JSON Schema / XSD，作适配资产、不进语言真源）；⑧ **§15-8 取 C**（**与助手建议的 8A 相反**）：**`Sink` 正式名改为 `EventSink`**（与 `EventSource` 对称）——**全仓现行文档已改名**（`glossary` §3.1/§3.2、`events.md` §2、`event_bus.md` §1.1/§4.1/§7.3、`ide/diagrams.md` §4、`index.md` 第 23 篇），**引用 Flink / Kafka 官方词处保留 `Sink`**；⑨ **§15-9 取 A + 补实义**：**HTTP 接口写「API / 接口」、集成连接写「端点」**（强调「集成端点」），且**端点 = API + 集成定义**——作者原话「端点是要在 API 的基础上增加定义数据的转化、过滤规则的」→ 端点在其上再加**数据转化（Converter / DataMapper）、过滤（Filter / Validator）、投递方式（Write / Push / Call）、触发与重试、对账**，**不改 API 业务语义**（落 `event_bus.md` §6 定义块 + `glossary` §3.1） | [`doc/event_bus.md`](doc/event_bus.md) **§15（四列台账）+ §1.1 / §4.1 / §5.3 / §6 / §7.2 / §7.3 / §9.1 / §9.3 / §10**、[`doc/glossary.md`](doc/glossary.md) §3.1 / §3.2、[`doc/events.md`](doc/events.md) §2、[`doc/ide/diagrams.md`](doc/ide/diagrams.md) §4、[`doc/operations.md`](doc/operations.md) §9、[`doc/errata.md`](doc/errata.md) §五-33 + §三-27 / §三-28 |
