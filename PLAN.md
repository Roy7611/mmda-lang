...
| 19 | **核心平台统一 MIT**（作者：「**核心平台 MIT**」） | 开源侧许可**全栈统一为 MIT**（`LICENSE`）：m 语言规范 + **Rust 内核 / 三端薄适配 / IDE 壳**——原记「规范 MIT / 内核 Apache-2.0」，现合为一份。理由：**集成方零摩擦、法务最简单**。**代价（要知道）**：MIT **不带专利授权与商标条款**——商标靠 README / 官网声明，专利风险靠防御性公开与自有专利布局 | [`doc/vision.md`](doc/vision.md) §5.1、[`doc/protection.md`](doc/protection.md) §7.1、[`doc/targets.md`](doc/targets.md) §8-5、[`doc/errata.md`](doc/errata.md) §五-20 |
