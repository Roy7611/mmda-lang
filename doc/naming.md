...
| 主键 / 外键 / 唯一约束名 | **不做硬约束**（由方言默认产出） | — |
| **索引 / 函数 / 存储过程** | **不做硬约束**；**建议前缀**：索引 **`IDX_`** · 函数 **`FUNC_`** · 存储过程 **`PROC_`** | `IDX_Material_materialCode` |
...
| 8 | ~~**主键 / 外键 / 索引 / 唯一约束名格式**~~ → **✔ 已裁（2026-09-24）：不做硬约束**；**只给前缀建议**——**索引 `IDX_`** · **函数 `FUNC_`** · **存储过程 `PROC_`** | ~~A 强制 `PK_` / `FK_` / `IX_` / `UX_` 格式~~ ／ ~~B 数据库默认名~~ ／ **不做硬约束 + 前缀建议** ✔ | **按作者口径**（前缀只是**建议**：`mmda check` 只出 **suggestion**，连 warning 都不算；主键 / 外键 / 唯一约束名**不约束**，由方言默认产出） |
