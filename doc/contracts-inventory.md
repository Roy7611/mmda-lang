...
> **✔ 再裁（同一日，收紧）**：UI 契约 = **现有 mmda-vue 前端项目**（`D:\2026\ts\mmda`）；**不考虑 C# MVC 与 Java 的 UI**；后端只提供 `MetaUi` 元数据。上表 C# 列的 `Mmda.Ui.*` 一律视为**遗留实现**（不纳入契约、不作生成目标、不随 mmda-lang 演进）。落点：[`presentation.md`](presentation.md) §5.1。
