...
| # | 口径 | 落法 |
| --- | --- | --- |
| 1 | **允许跨模块调用** | 脚本可按模块调用任意 module 的**声明接口**；**事务按已裁规则走**（§3 / §9-7）：**同模块同事务、跨模块新事务**——脚本里跨模块写**不与本模块同一事务**，要最终一致就靠事件补偿（[`event_bus.md`](event_bus.md) §9.3 Outbox） |
...
**⏳ 仍待裁一条：脚本的运行身份**——脚本里读写的权限按谁算？

- **A 继承调用者**：脚本以触发它的调用者的权限与数据范围运行，越权同样被拒（安全默认）；
- **B 声明运行身份**（`runAs`）：定时任务 / 事件消费者等无调用者的场景以 `system` 身份运行；
- **C 默认继承 + 允许声明 `system`（助手建议）**：默认 A；声明 `runAs: system` 必须**评审并在安全清单可见**、进 `mmda check` warning（否则「能访问所有 Entity」等于给业务人员开了一把越权后门，OWASP ASVS 的访问控制项会失守）。
...
| 事件 | `script.event.publish(事件, 载荷)` | 走 **Outbox**（与业务同一事务，[`event_bus.md`](event_bus.md) §9.3） |
