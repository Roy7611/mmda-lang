...
> ✔ **两个「钩子的家」分清楚（2026-09-24，作者补充原话：「实际上 service 层也有钩子，是给业务逻辑用的」）**：**`beforeXxx` / `afterXxx` 住 Service**（业务逻辑，与业务同一事务，§3）；**`store` 级钩子住 Repository**（存储语义，见 §4.4——**已裁不开放**）。**钩子里能写什么**见 §4.5（⏳ 待裁）。
...
### 4.5 钩子里能写什么：宿主代码 / 声明式 / 受限脚本（⏳ 待裁）

> 作者原话：「**实际上 service 层也有钩子，是给业务逻辑用的**。我原先想能否有**脚本执行引擎**，嵌入到 java、c# 后台，能够写一些简单的脚本，**起到类似数据库的触发器的作用**。技术上有没有可能？**数据库的触发器没有可移植性，换数据库就废了**」

先把两件事分开：

- **钩子住哪（✔ 已裁）**：`beforeXxx` / `afterXxx` 住 **Service**（业务逻辑、与业务同一事务，§3）；`store` 级钩子住 **Repository**（存储语义），**取 A：不对设计师开放**（§4.4）。
- **钩子里写什么（本节，⏳ 待裁）**——这是新问题。
...
| 平台 | 现成方案 | 硬事实（选型必须知道） |
| --- | --- | --- |
| Java（JDK 21） | **Nashorn 已随 JDK 15 移除**（JEP 372），要自己带引擎[^jep372]；**GraalJS** 有 Maven 包（`org.graalvm.polyglot:polyglot` + `:js`，JSR-223 名 `Graal.js`）[^graaljs] | GraalJS **能在普通 OpenJDK 上跑，但官方口径是「stock JVM 不受支持」**——没有 Graal 编译器时退化为解释执行，性能与兼容性都打折；要性能就得换 GraalVM JDK（**与 L2 基线的毕昇 JDK 21 冲突**，[`targets.md`](targets.md)） |
| .NET | `Microsoft.CodeAnalysis.CSharp.Scripting`（Roslyn 脚本）[^roslyn]、**Jint**（纯 .NET JS 解释器，有 `Engine.PrepareScript` 缓存与执行限额）[^jint]、ClearScript（V8） | **脚本引擎的「限制」不是隔离边界**——Jint 文档明说：`Object.prototype` 污染等状态会在同一引擎内残留，**互不信任的脚本仍需独立引擎/进程** |

**三条路（助手建议 A）**：

- **A（建议）受限脚本 = 自己的内核求值器**：钩子里的脚本是 **m 语言的受限子集**，由 **Rust 内核**解析求值，三端经**已有的嵌入通道**调用（**P6：Panama / P-Invoke + WASM**，见 [`PLAN.md`](../PLAN.md) §4）。
  - 好处：**三端同一套语义**（一个真源、一份测试，不用 Java 写一遍 C# 再写一遍）、**沙箱是默认值**（无文件系统 / 无网络 / 无反射）、**可静态校验**（进 `mmda check`）、**随元数据版本走**（触发器没有版本控制、没有 diff、没有评审——这条正是我们要替掉的）、与已裁的「**支持 m 语言写逻辑**」（§9-1）是同一件事的延伸。
  - 代价：脚本表达力**故意受限**（**纯函数 + 受限数据访问**：只能经 `EntityFactory` / `Repository` 声明里已知的对象读写，禁任意 IO / 网络 / 随机 / 时间依赖，与 [`events.md`](events.md) 表达式层的判据一致）；复杂逻辑仍写宿主代码（KEEP 区）。
- **B 宿主脚本引擎**（Java GraalJS／C# Roslyn 或 Jint）：程序员最熟、表达力最强；代价 = **同一个钩子三端各写一遍**（语义漂移）、依赖变重（GraalJS 在 stock JDK 上不受支持）、沙箱与安全各自处理、脚本无类型校验，且宿主运行时升级就要重测。
- **C 不做脚本**：只保留声明式（`beforeXxx` 点 + Converter / Filter / Validator / 条件表达式，均已裁）——最省事，但覆盖不了「临时 / 客户特有」的触发器式逻辑，这类需求会被迫回到 DB 触发器（**正是本条要消灭的东西**）。

**「替代数据库触发器」这半句其实已经有答案**：MMDA 的可移植版本就是 **拦截点（§4.2）+ Outbox（事务性发件箱，[`event_bus.md`](event_bus.md) §9.3）**——业务事件随业务事务落库、由中继投递，换库不废。**脚本只补最后一环**：让这些钩子**不必写宿主代码**。

**待裁问题**：① 取 **A / B / C**？② 若取 A，**首版脚本能力清单**要定到哪一档（哪些内置函数、哪些能力须由 capability 声明、是否允许跨模块调用）。
...
[^roslyn]: NuGet `Microsoft.CodeAnalysis.CSharp.Scripting`。
[^jint]: Jint（`sebastienros/jint`）：`Engine.PrepareScript` 复用、执行限额；文档声明其配置复用「**不是隔离边界**」。
