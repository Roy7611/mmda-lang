...
**✔ 预留宿主（2026-09-26 作者：「rust目前不实现，未来可能，你作为架构师不会这么想？」）**：**上面这三端是「现在在做的」，不是「语言只认这三家」**。**宿主清单是数据、不是语法** —— 一个宿主 = **Profile 里 `target <name>` 声明一个投影器**（消费 IR、生成该语言产物、回答 8 轴），**加 Rust（或其他语言）只增一行数据：不改语言、不改类型系统、不改映射规则**（承 [`datatypes.md`](lang/datatypes.md) §11.2 投影器口径）。**现状不虚报**：`PLAN.md` §0 **B6 仍是「Java / C# / TS」三个实现目标**；**Rust 是预留位**（未来若做第 4 个实体类宿主，实体生成 + ORM：diesel / sqlx / sea-orm 由 Profile 声明）。**纪律**：类型层 / 映射层不许出现 `if (target == "…")` 式的硬编码分支。
...
3. **判定**：同一用例在三端跑，**产物必须一致**——字段序列化、校验错误码、状态转移的合法/非法结果、事件投递次数与顺序、生成 DDL 的规范化文本、多租户隔离的数据可见性、**标识符命名（类型名 / 属性名 / 事件名 / 消息头名逐字对账，口径见 [`naming.md`](naming.md) §2；DDL 侧只对账表 / 视图 / 列——**约束名与索引名是建议项、不进对账**，见 §3.3）**。
