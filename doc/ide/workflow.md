...
点击树节点 → **属性面板** 编辑上述字段。详见 [meta-model.md](../meta-model.md) 与根目录设计笔记中的 TypeScript 示例。
...
实体通过 `IAuthorizable` 约定 `creatorId`、`ownerId` 等字段支撑数据范围。资产目录：`flow/roles/*.mr`（见 [doc/project.md](../project.md)）。
