...
**作者口径（原话，2026-09-24）**：「**共赢做插件市场，留这个口子，设计阶段原生支持插件式开发**」。**⚠️ 同日再补**：**市场与收费暂缓**（「目前不考虑收费和市场」）；**「留口子」= 设计阶段原生支持继续做**（本节 + [`../runtime.md`](../runtime.md) §7.5），市场机制只留方向（§9 抬头）。
...
## 9. 插件市场（⏸ 2026-09-24 起暂缓：**目前不考虑收费和市场**）

### 9.1 定位（共赢的机制）

> **⏸ 暂缓（2026-09-24，作者原话）**：「**业务插件是指用户用 java, c#, ts 做的，目前不考虑收费和市场**」。本节保留为**方向与口子**（机制设计不删），但**不进首版、不进任何版本承诺**；真要启动市场时以本章为起点。
...
## 12. 插件加载形态（✔ 已裁 2026-09-25：内核侧插件取 B = WASM 组件）
...
**✔ 已裁（2026-09-25，作者取 B）：内核侧插件 = WASM（wasmtime + component model）**。

理由：**沙箱最强 + 确定性 + 跨平台分发**，而且**绕开了 C 的死结**（Rust 无稳定 ABI）——插件是 `.wasm` 组件、**与内核编译器版本解绑**。
**代价照记**：① 插件**只能用能编译到 WASM 的语言**（Rust / C / C++ / TinyGo / AssemblyScript…）；② **文件 / 网络 / 进程必须由宿主函数按能力白名单暴露**（与内核 capability 机制同一条规矩，wasm 组件默认碰不到宿主）。

**A（sidecar + JSON-RPC）降为备选口子**——将来若真要用**无法编译到 WASM 的语言**写内核侧插件，仍走它；**C（Rust dylib）仍只作官方内置扩展的实现方式、不对外**（落点 [`../errata.md`](../errata.md) §五-41）。
