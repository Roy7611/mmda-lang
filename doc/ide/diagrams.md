...
### 10.4 ✔ 已裁（2026-09-25 作者：「**5 条都按你建议**」）

1. **命令面 = 两条命令**：**`mmda doc`**（模型文档）+ **`mmda diagram`**（图）—— **不**统一到 `mmda export`（`export` 已被 API / DDL 侧占用）；
2. **首版格式集合 = Mermaid + PlantUML**：Mermaid 必做（GitHub / GitLab / VS Code / Obsidian 原生渲染）、PlantUML 次做（企业文档常用、需渲染服务）；**D2 / Graphviz dot 留口子**（后续后端，不进首版）；
3. **文档结构 = 每对象一节（字段表 + 关系图）**；**UI 五视图与验收用例不进正文**，走 **`--with ui,tests`** 附录开关；**文档首页 = 模块树小节**（不另出「一页总览」图）；
4. **模板 = 固定骨架 + 预留模板文件覆盖**：首版出固定骨架（保证 `diff = 0` 可复现），模板文件覆盖的口子留着、后续实现；
5. **不反哺布局**：导出是**单向产物**（保证 `diff = 0` 干净）；确需导入只作**一次性初始布局**，不进一致性口径。
