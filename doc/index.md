...
| [guide/enums.md](guide/enums.md) | **枚举开发指南**：一分钟模板、命名与文件、注释规范（`/// label : description`，写在上一行）、值域与位枚举、`@State` 与状态机、**外观注解**（颜色 `@Colorized` / `@Color` + 7 角色 + 封闭 10 档深度；图标 `@Iconized` / `@Icon` 完整别名）、回落规则表、自检清单、常见坑、迁移、三端落地 |
