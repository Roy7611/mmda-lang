...
| `@Colorized` / `@Iconized` | **枚举的呈现开关**（✔ 2026-09-25）：写在**枚举声明**上，开**颜色** / **图标**渲染；具体取值在成员上（`@ColorRole` / `@Icon`）；没开开关不渲染 |
| `@ColorRole(role)` | **枚举成员的主题色角色**（✔ 2026-09-25）：**封闭 6 值** `primary` / `secondary` / `info` / `success` / `warning` / `danger`（语言层校验，拼错 = 解析期 error）；**只给角色不给色值**（色值来自主题令牌，模型层不写 `#RRGGBB`） |
