...
| `@ColorRole(role)` | **枚举成员的主题色角色**（✔ 2026-09-25）：`primary` / `secondary` / `info` / `success` / `warning` / `danger`；**只给角色不给色值**（色值来自主题令牌，模型层不写 `#RRGGBB`） |
| `@Icon("alias")` | **枚举成员的图标别名**（✔ 2026-09-25）：逻辑别名（如 `"cancel"`），**不绑定图标库**，由各端主题映射成具体图标 |
