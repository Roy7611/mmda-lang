...
| `@Colorized` / `@Iconized` | **枚举的呈现开关 + 默认值**（✔ 2026-09-25）：写在**枚举声明**上；`@Colorized(role, depth?)` 只负责给**默认色**（成员 `@Color` 缺省时回落）、`@Iconized`（**无参、不写括号**）默认别名 = **成员名 kebab**、`@Iconized("bom")` 默认别名 = **前缀 + 成员名 kebab**（`bom-design`）；成员不写就用默认，没默认才不渲染 |
| `@Color(role, depth?)` | **枚举成员的主题色**（✔ 2026-09-25；原 `@ColorRole(role)` 写法并入）：**封闭 7 值** `primary` / `secondary` / `info` / `success` / `warning` / `danger` / `gray`（含**默认支持的黑白灰**；语言层校验，拼错 = 解析期 error）+ **色板深度**（**封闭 10 档** `50`–`900`，**省略 = `500`**）；**只给角色与深度、不给色值**（色值来自主题令牌，模型层不写 `#RRGGBB`） |
