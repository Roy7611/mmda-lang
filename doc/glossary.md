...
| 字符串表示（旧列名 `enumString`） | `MetaEnum.toString()` / `fromString()` 的成员串（**不是 JSON 字段**）：老格式 `value;name;text`，✔ 2026-09-25 扩展为 `value;name;text;color;icon`（后 2 段可省、空段 = 未声明；**颜色段 = `<role>-<shade>`**，如 `info-500`）——⚠️ 老运行时 `split(';',3)` 会把第 3 段吞成整串，故扩展段只在用外观注解时产出、需与同代内核配套 |
