...
| `@Colorized` / `@Iconized` | **枚举的呈现开关 + 默认值**（✔ 2026-09-25）：写在**枚举声明**上；`@Colorized(role, depth?)` 给**默认色**、`@Iconized(default)` 表示**默认别名取成员名**、`@Iconized("bom")` 表示**默认别名加前缀**（`bom-design`）；成员不写就用默认，没默认才不渲染 |
| `@Color(role, depth?)` | **枚举成员的主题色**（✔ 2026-09-25；原 `@ColorRole(role)` 写法并入）：**封闭 7 值** `primary` / `secondary` / `info` / `success` / `warning` / `danger` / `gray`（含**默认支持的黑白灰**；语言层校验，拼错 = 解析期 error）+ **色板深度**（Material 十档 `50`–`900`）；**只给角色与深度、不给色值**（色值来自主题令牌，模型层不写 `#RRGGBB`） |
| `@Icon("alias")` | **枚举成员的图标别名**（✔ 2026-09-25）：逻辑别名（如 `"cancel"`），**开放** —— 语义词由**开发人员自定义**，语言层只校验是字符串；**不绑定图标库**，映射在 **UI 层**（主题 / 皮肤）；**没映射上顶多不显示**（可选 warning，不报错） |
...
| 标识共享（Identity Sharing） | **一组要 UNION 成同一个视图的表，在 realId 空间里各领一个互不重叠的段**（`@Partitioned [min,max]` 声明），使 UNION 后的主键天然不冲突（✔ 2026-09-25 作者说明；段划分与六个组见 [`records.md`](records.md) §2.3） |
| `BIGID` | **自有整数类型（文档用名）**：本身即 partitionID（分区主键）—— **高 28 位字段 = tenantId（27 位有效，`0x7FF_FFFF`，bit 63 保留恒 0）+ 低 36 位 realId**（解析 `>>> 36`；`0` = 无租户，`1` 起为真实租户；realId 由分布式 ID 生成、底座合成）。**语料写法 = `@Partitioned [min,max]` + `uint64 identity`**，见 [`datatypes.md`](datatypes.md) §5 |
