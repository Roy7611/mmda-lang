...
| 24 | **愿景类 5 条**：① 规范文本许可最终选型（MIT vs CC-BY-4.0）；② **移动端（多端）** 是否进首版承诺及宿主形态；③ 安全目标是否引入 **OWASP ASVS / Top 10** 作附加硬门禁；④ **L2 国产化的目标矩阵清单**（哪些 OS 版本 / CPU 架构 / JDK 发行版进首版验收）；⑤ 「共赢」是否落成**插件市场 / 伙伴体系** | [`vision.md`](vision.md) **§8（5 条，逐条给建议）** |
