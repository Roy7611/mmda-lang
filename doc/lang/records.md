...
| `asc` / `desc` | **`@Index` 列级排序**（不写 = `asc`；大小写不敏感）：`@Index IDX_x(createdAt desc, status)`；**空值位置语言层写死「排最前」（`NULLS FIRST`）**，**不引入 `nulls first` / `nulls last` 关键字**（写了 = 解析期 error；✔ 2026-09-26 作者：「asc / desc 肯定要，默认 nulls first，不引入」） |
| `identity` | 唯一标识；等价 `indexed` + `unique`；默认配 `readonly`；**可带起点 / 步长 = `identity(start, step)`**（✔ 2026-09-26 作者：「**同意**」，写法 = 助手提案）—— **缺省 `identity` = `identity(1, 1)`**（✔ 2026-09-26 作者：「**支持，但是不写默认 `(1,1)`**」—— 早前文档写的「缺省 = 数据库默认」**更正**）；DB 侧：**SQL Server `IDENTITY(start, step)` 原生**、MySQL `AUTO_INCREMENT` 起点、Oracle / PG 走 `SEQUENCE START WITH … INCREMENT BY …`（由 `identity` 列投影，细节随 Profile） |
...
| `default` *value* | 默认值；`now` = 当前时间；**布尔写 `true` / `false`**（**大小写不敏感**，`1` / `0` 等价；`b'0` / `b'1` 是 MySQL 语法、**不要**）；**可空字段不写 = `null`，显式 `default null` 也认**；**枚举写成员名（`default NONE`）或成员值（`default 0`）都认**（`mmda fmt` 归一写成员名）；**位串字面量：`BitStr` 用 `b'0110'`、`BitVector` 用 `0b…`** |
